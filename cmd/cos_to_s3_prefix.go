...
// COSToS3Prefix 将 COS 源桶指定前缀下所有对象复制到 S3 目标桶的指定前缀下
func COSToS3Prefix(ctx context.Context, cfg COSToS3PrefixConfig) error {
	cos := cosClient.New(cfg.COSSecretID, cfg.COSSecretKey, cfg.SrcBucket, cfg.SrcRegion)

	log.Printf("[cos-to-s3 prefix] 列举 cos://%s/%s ...", cfg.SrcBucket, cfg.SrcPrefix)
	keys, err := cos.ListObjects(ctx, cfg.SrcPrefix)
	if err != nil {
		return err
	}
	...
	for _, key := range keys {
		...
		go func() {
			...
			size, err := cos.HeadObject(ctx, key)
			if err != nil {
				mu.Lock()
			...
			err = s3.MultipartUpload(ctx, dstKey, size, chunkSize, cfg.ChunkConcurrency,
				func(partNumber int, offset, sz int64) ([]byte, error) {
					data, err := cos.GetRange(ctx, key, offset, offset+sz-1)
					if err == nil {
						prog.Add(sz)
					}
					return data, err
...
type COSToS3PrefixConfig struct {
	COSSecretID  string
	COSSecretKey string
	SrcBucket    string
	SrcRegion    string
